...
import com.baka3k.core.model.upsert
import com.baka3k.data.movie.database.dao.MovieType.NOW_PLAYING
import com.baka3k.data.movie.database.dao.MovieType.OTHER
import com.baka3k.data.movie.database.dao.MovieType.POPULAR
import com.baka3k.data.movie.database.dao.MovieType.TOP_RATE
import com.baka3k.data.movie.database.dao.MovieType.UP_COMMING
...
interface MovieDao {
    ...
    @Query(
        value = """
            SELECT * FROM movies INNER JOIN movie_type_cross_ref ON movies.id = movie_type_cross_ref.movie_id
            WHERE movie_type_cross_ref.type_id = '$OTHER'
    ...
    @Query(
        value = """
            SELECT * FROM movies INNER JOIN movie_type_cross_ref ON movies.id = movie_type_cross_ref.movie_id
            WHERE movie_type_cross_ref.type_id = '$POPULAR'
            ORDER BY movies.id  DESC
    """
    )
    ...
    @Query(
        value = """
            SELECT * FROM movies INNER JOIN movie_type_cross_ref ON movies.id = movie_type_cross_ref.movie_id
            WHERE movie_type_cross_ref.type_id = '$NOW_PLAYING'
            ORDER BY movies.id  DESC
    """
    )
    ...
    @Query(
        value = """
            SELECT * FROM movies INNER JOIN movie_type_cross_ref ON movies.id = movie_type_cross_ref.movie_id
            WHERE movie_type_cross_ref.type_id = '$TOP_RATE'
            ORDER BY movies.id  DESC
    """
    )
    ...
    @Query(
        value = """
            SELECT * FROM movies INNER JOIN movie_type_cross_ref ON movies.id = movie_type_cross_ref.movie_id
            WHERE movie_type_cross_ref.type_id = '$UP_COMMING'
            ORDER BY movies.id  DESC
    """
    )
    ...
    @Query(
        value = """
             DELETE FROM movies WHERE movies.id in (SELECT movie_id From movie_type_cross_ref WHERE type_id = '$OTHER')
        """
    )
